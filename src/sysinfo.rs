...
pub fn get_ram() -> MemInfo {
    ...
    let (mem_total,mem_free) = scan_fmt!(&meminfo, //input string
                                                "MemTotal:         {d} kB\nMemFree:          {d} kB", //format
                                                u32,u32).unwrap(); //types
...
fn get_cpu_occupy() -> CpuOccupy {
    ...
    let (user,nice,system,idle,iowait,irq,softirq) = scan_fmt!(&stat_info, //input string
                                                "cpu  {d} {d} {d} {d} {d} {d} {d} ", //format
                                                u32,u32,u32,u32,u32,u32,u32).unwrap(); //types
...
fn cal_cpuoccupy(c1: CpuOccupy,c2: CpuOccupy) -> f32 {
    ...
    if (nd - od) != 0.0 {
        cpu_use =(1_f32 -  (id - sd)/(nd - od))*100.0;
        println!("cpu_use = {}",cpu_use);
    } else {
        cpu_use = 0.0;
    }

    return cpu_use.round_fixed(2);
}
