...
fn main() {
    ...
    let cpu_usage = sysinfo::get_sys_cpu_usage().to_string();

   // println!("MemTotal {} kB,MemFree {} kB",meminfo.total,meminfo.free);

    let ip_info_show = "IP: ".to_owned() + &rpi_ip;
    let meminfo_show = "RAM: ".to_owned() + &((meminfo.total - meminfo.free)/1024).to_string() + " / " +  &(meminfo.total/1024).to_string() + " MB";
